...
export const socialRouter = router({
  ...
  analyzeFeedSentiment: publicProcedure
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        const analysisResults = Array.from(sentimentAnalysis.entries()).map(
          ([postId, analysis]) => ({
            postId,
            ...analysis,
        ...
        const avgRelevanceScore =
          analysisResults.reduce((sum, a) => sum + a.relevanceScore, 0) / analysisResults.length;
        // Extract top topics (simplified for now)
        const topTokens = analysisResults.map((a) =>
          a.relevanceScore > 0.5 ? 'proptech' : 'general'
        );
  ...
  generateContent: publicProcedure.input(generateContentSchema).mutation(async ({ input }) => {
    try {
      ...
      const toneTemplates =
        focusTemplates?.[input.tone as keyof typeof focusTemplates] ||
        focusTemplates?.['professional' as keyof typeof focusTemplates] ||
        [];
  ...
  discoverLeads: publicProcedure.input(leadDiscoverySchema).mutation(async ({ input }) => {
    try {
      ...
      const qualifiedLeads = discoveredLeads.filter(
        (lead) =>
  ...
  getPendingPosts: publicProcedure.query(async () => {
    try {
  ...
  reviewPost: publicProcedure.input(postApprovalSchema).mutation(async ({ input }) => {
    try {
      ...
      try {
        ...
        return {
          ...
          summary: {
            ...
            avgEngagement:
              leadAnalytics.reduce((sum, l) => sum + (l.avgEngagement || 0), 0) /
              leadAnalytics.length,
          },
        };
      } catch (error) {
