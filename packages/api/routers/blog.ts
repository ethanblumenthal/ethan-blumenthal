...
export const blogRouter = router({
    ...
    .query(async ({ input }) => {
      try {
        ...
        if (tags && tags.length > 0) {
          ...
          conditions.push(or(...tags.map((tag) => sql`${blogPosts.tags} @> ${[tag]}`)));
        }
  ...
  create: publicProcedure.input(createBlogPostSchema).mutation(async ({ input }) => {
