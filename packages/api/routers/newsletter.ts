...
export const newsletterRouter = router({
  // Newsletter signup
  signup: publicProcedure.input(newsletterSignupSchema).mutation(async ({ input }) => {
    ...
    emailService
      ...
      .catch((error) => console.error('Failed to send newsletter welcome email:', error));
  ...
  getStats: publicProcedure.query(async () => {
    ...
    const recentSignupsCount = await db
      ...
      .from(contacts)
      .where(
        sql`${contacts.notes} LIKE '%Newsletter signup%' AND ${contacts.createdAt} > NOW() - INTERVAL '30 days'`
      );
